...
size_t get_bytes_len(const Amount& amount)
{
    return amount.get_exported_size_in_bytes();
}
...
IntWrapper<uint8_t> as_uint8(const Amount& value)
{
...
EthereumDataStream& operator<<(EthereumDataStream& stream, const IntWrapper<Amount>& wrapper)
{
    const BinaryDataPtr data = wrapper.value.export_as_binary_data(Amount::EXPORT_BIG_ENDIAN);
    if (wrapper.size > data->len)
    {
        THROW_EXCEPTION("Can't serialize Amount as fixed-size blob.")
...
std::string to_string(const Amount& amount)
{
...
EthereumDataStream& operator<<(EthereumDataStream& stream, const Amount& data)
{
    write_int(data, &stream);
    return stream;
}
...
struct EthereumTransactionFee : public TransactionFeeBase
{
    ...
    PropertyT<Amount> gas_price;
    PropertyT<Amount> gas_limit;
...
BinaryDataPtr EthereumTransaction::serialize() const
{
...
Amount EthereumTransaction::get_total_fee() const
{
...
void EthereumTransaction::update_state()
{
    ...
    if (!m_fee->gas_price.is_set())
    {

    }
    Amount diff = m_source->amount.get_value() - m_destination->amount.get_value();
    m_gas = *m_fee->gas_limit;

    diff -= m_gas * *m_fee->gas_price;
    if (diff < 0)
    {
...
Amount EthereumTransaction::estimate_total_fee(size_t sources_count, size_t destinations_count) const
{
