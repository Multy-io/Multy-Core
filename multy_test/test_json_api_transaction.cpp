...
TEST_P(JsonTransactionApiTestP, make_transaction_from_json)
{
    ...
    ConstCharPtr actual_tx(
            make_transaction_from_json(param.json_request)
    );
...
TEST_P(JsonTransactionApiErrorTestP, make_transaction_from_json)
{
    ...
    const Json::Value err = parse_json(output.get())["error"];
    if (param.expected_error_message)
    {
        SCOPED_TRACE(param.expected_error_message);
        EXPECT_TRUE(std::regex_match(err["message"].asCString(), std::regex(param.expected_error_message)));
    }

    if (param.expected_error_code > 0)
    {
        ASSERT_EQ(param.expected_error_code, err["code"].asInt());
    }
}
