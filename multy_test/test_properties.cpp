...
GTEST_TEST(PropertiesTestInvalidArgs, properties_set_int32_value)
{
    ...
    error.reset(properties_set_int32_value(&properties, "v", 2));
    EXPECT_NE(nullptr, error);

    properties.bind_property("v", &int_property);

    error.reset(properties_set_int32_value(&properties, "", 1));
    EXPECT_NE(nullptr, error);
    EXPECT_EQ(0, int_property);

    error.reset(properties_set_int32_value(&properties, nullptr, 1));
    EXPECT_NE(nullptr, error);
    EXPECT_EQ(0, int_property);

    error.reset(properties_set_int32_value(nullptr, "v", 1));
    EXPECT_NE(nullptr, error);
    EXPECT_EQ(0, int_property);
}
...
GTEST_TEST(PropertiesTestInvalidArgs, properties_set_amount_value)
{
    ...
    error.reset(properties_set_amount_value(&properties, nullptr, &invalid_amount));
    EXPECT_NE(nullptr, error);
    EXPECT_STREQ("0", amount_property.get_value().c_str());
...
GTEST_TEST(PropertiesTestInvalidArgs, properties_set_binary_data_value)
{
    ...
    BinaryDataPtr binary_data_check_not_nullptr(make_clone(BinaryData{data_2_vals, 2}));

    error.reset(properties_set_binary_data_value(&properties, "v", &binary_data2));
    EXPECT_NE(nullptr, error);
    EXPECT_EQ(nullptr, binaty_data_property);

    properties.bind_property("v", &binaty_data_property);

    error.reset(properties_set_binary_data_value(&properties, nullptr, &binary_data2));
    EXPECT_NE(nullptr, error);
    EXPECT_EQ(nullptr, binaty_data_property);
...
GTEST_TEST(PropertiesTestInvalidArgs, properties_set_public_key_value)
{
    ...
    AccountPtr account1;
    make_account(CURRENCY_BITCOIN,
        "cScuLx5taDyuAfCnin5WWZz65yGCHMuuaFv6mgearmqAHC4p53sz",
        reset_sp(account1));

     pub_key_property1 = account1->get_public_key();

    error.reset(properties_set_public_key_value(&properties, "v",  pub_key_property.get()));
    EXPECT_NE(nullptr, error);
    EXPECT_EQ(nullptr,  pub_key_property);

    properties.bind_property("v", &pub_key_property);

    error.reset(properties_set_public_key_value(&properties, nullptr,  pub_key_property1.get()));
    EXPECT_NE(nullptr, error);
    EXPECT_EQ(nullptr,  pub_key_property);

    error.reset(properties_set_public_key_value(nullptr, "v",  pub_key_property1.get()));
    EXPECT_NE(nullptr, error);
    EXPECT_EQ(nullptr,  pub_key_property);
...
GTEST_TEST(PropertiesTestInvalidType, int32_properties)
{
    ...
    const BinaryDataPtr binaty_data_4_property(make_clone(BinaryData{data_4_vals, 4}));
    ...
    error.reset(properties_set_binary_data_value(&properties, "v", binaty_data_4_property.get()));
    EXPECT_NE(nullptr, error);
    EXPECT_EQ(1, int_value);
...
GTEST_TEST(PropertiesTestInvalidType, BinaryData_properties)
{
    ...
    BinaryDataPtr binaty_data_4_property(make_clone(BinaryData{data_4_vals, 4}));
...
GTEST_TEST(PropertiesTest, properties_set_binary_data_value)
{
    ...
    const BinaryDataPtr binaty_data_2_property(make_clone(BinaryData{data_2_vals, 2}));

    properties.bind_property("v", &binaty_data_property);

    error.reset(properties_set_binary_data_value(&properties, "v", binaty_data_2_property.get()));
    EXPECT_EQ(nullptr, error);
    EXPECT_EQ(reference_data_2_value, *binaty_data_property);

    error.reset(properties_set_binary_data_value(&properties, "v", binaty_data_property.get()));
    EXPECT_EQ(nullptr, error);
...
GTEST_TEST(PropertiesTest, properties_set_public_key_value)
{
    ...
    error.reset(properties_set_public_key_value(&properties, "v",  pub_key_property1.get()));
    EXPECT_EQ(nullptr, error);
...
GTEST_TEST(PropertiesTest, properties_get_specification)
{
    ...
    error.reset(properties_get_specification(&properties, reset_sp(specification)));
    EXPECT_EQ(nullptr, error);

    EXPECT_NE(nullptr, strstr(specification.get(), "MANY_LONG_STRING_FOR_CHECKING"));
}
