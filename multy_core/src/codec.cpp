...
public:
    ...
    virtual std::string encode(const BinaryData& binary) = 0;
    virtual BinaryDataPtr decode(const char* str, size_t len) = 0;
};

class HexCodec : public CodecBase
{
    ~HexCodec()
    {}

    std::string encode(const BinaryData& binary) override
    {
    ...
    BinaryDataPtr decode(const char* hex_str, size_t len) override
    {
...
class Base58Codec : public CodecBase
{
    ~Base58Codec()
    {}
    ...
    BinaryDataPtr decode(const char* str, size_t len) override
    {
        BinaryDataPtr result;
        size_t resulting_size = len;

        throw_if_error(make_binary_data(resulting_size, reset_sp(result)));
        THROW_IF_WALLY_ERROR(
...
typedef std::unique_ptr<CodecBase> CodecPtr;
CodecPtr make_codec(CodecType codec_type)
{
    switch(codec_type)
    {
        case CODEC_HEX:
            return CodecPtr(new HexCodec);
        case CODEC_BASE58:
