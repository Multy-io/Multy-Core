...
{
    Json::Value error_root = make_json_object({
        {"code", error.code},
        {"message", error.message},
        {"location", make_json_object({
    ...
    if (error.backtrace)
    {
        error_root["backtrace"] = error.backtrace;
    }

    return to_string(error_root);
}

std::string exception_to_json(const std::exception& exception)
{
    return to_string(make_json_object({
